...
from .Invoke import user_function as Invoke_function
import cpuinfo
import sys
import objsize
import random
...
def main(serwoObject, context: az_func.Context) -> str:
    try:
        ...
        if isinstance(serwoObject, list):
            ...
            serwoObjectResponse = Invoke_function(serwo_list_object)
            ...
            serwoObjectResponse = Invoke_function(serwoObject)
            body_after = serwoObjectResponse.get_body()
            output_body_size = objsize.get_deep_size(body_after)
            process = psutil.Process(os.getpid())
            memory = process.memory_info().rss
